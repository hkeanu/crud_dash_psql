...
@app.callback(
    ...
    [State('rem-scan-input', 'value'),
     Input('our-table', 'data'),
     Input('float-column-dropdown', 'value')],  # Add the dropdown as an input
    prevent_initial_call=True
)
def display_rem_scan_graph(n_clicks, rem_scan_serial, data, selected_float_column):
    if not data or not rem_scan_serial or not selected_float_column:
        return {'data': []}

    df_fig = pd.DataFrame(data)

    # Filter the data based on the selected RemScan Serial
    filtered_data = df_fig[df_fig['RemScan_Serial'] == rem_scan_serial]
    ...
    fig.update_layout(
        title=f'Data for RemScan Serial: {rem_scan_serial}',
